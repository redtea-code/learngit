...
1. Run `fetch_logos.py` (command above). It prints a **✓/✗ CHECKLIST** to stderr and a JSON object `{"logos": [...], "missing": ["Institute A", ...]}` to stdout.
   ...
   The manifest is the only institution-logo allowlist. Old files may remain in the directory for recoverability, but downstream code must ignore any file absent from the approved `logos[]` list.
2. **Read the `"missing"` array.** For **EVERY** name in it, run the web-search fallback — this is required, not optional:
   ...
   - Download it through the SAME pipeline (autotrim + canonical slug) so it matches the other tiles:
     ...
     Each fallback candidate goes through the same decode, light-header contrast, photo/cover, and visual-fingerprint gates as Wikimedia candidates. `--add-logo` atomically merges the accepted result into the existing manifest, replaces the prior selection for the same canonical institution, removes that institution from `missing`, and records rejected candidates separately. It never resets unrelated approved entries.
